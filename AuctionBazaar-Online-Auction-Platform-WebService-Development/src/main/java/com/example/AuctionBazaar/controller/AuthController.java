...
public class AuthController {
	...
	public ResponseEntity<Map<String, String>> login(@RequestBody LoginRequest loginRequest) {
		...
		if (userOpt.isEmpty()) {
			...
			return ResponseEntity.status(HttpStatus.NOT_FOUND).contentType(MediaType.APPLICATION_JSON) // Ensure JSON
																										// type
					.body(response);
		}
		...
		if (BCrypt.checkpw(loginRequest.getPassword(), user.getPasswordHash())) {
			...
			return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON) // Ensure JSON type
					.body(response);
		} else {
			// Password didn't match
			response.put("error", "Invalid credentials.");
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).contentType(MediaType.APPLICATION_JSON) // Ensure JSON
																											// type
					.body(response);
		}
	}
